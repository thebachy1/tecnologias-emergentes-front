...
import "./OficiosCambio.css";

export default function OficiosCambio() {
  ...
  const [apiUrl, setApiUrl] = useState(`http://127.0.0.1:8000/item/change/`);
  ...
  const searchHandler = () => {
    setApiUrl((apiUrl) =>
      apiUrl.concat(
        `${id}?name=${name}&apellido1=${apellido1}&apellido2=${apellido2}&cargo=${cargo}&empresa=${empresa}&calle=${calle}&numCalle=${numCalle}&colonia=${colonia}&municipio=${municipio}&estado=${estado}&codigoPostal=${codigoPostal}&telefono=${telefono}&email=${email}&yearNacimiento=${yearNacimiento}&monthNacimiento=${monthNacimiento}&dayNacimiento=${monthNacimiento}`
      )
    );
  };
